...
// click rather than a lap of the ring. Each of those pairs is then followed
// by its own seconds twin -- 24-hour, 12-hour, 24-hour+seconds, 12-hour+
// seconds -- so the AM/PM invariant holds inside the seconds pair too, and
// dropping to seconds from either twin is a single click past its own pair
// rather than a hop across the whole ring. The ISO preset is deliberately
// left without an AM/PM or seconds twin: ISO 8601 writes time on a 24-hour
// clock, so a variant of either kind would contradict the only thing that
// format is for.
var CLOCK_FORMATS = [
  "dddd HH:mm",
  "dddd h:mm AP",
...
// time carries it here; seconds costs only a third line on the compact
// two-line entry, so it gets its own step right after it instead.
